...
'   0.1      2018/02/15  (NES)小林  高機能化
' **********************************************************************
'TODO: 運管サーバ本体の改造の際に、これを本体用のソースにすること。
'また、ServerTelegrapherも同様に変更した方がよい。
Option Explicit On
Option Strict On
...
#Region "イベント処理メソッド"
    ...
    Protected Overridable Function ProcOnActiveDownloadResponseReceive(ByVal oRcvMsg As InternalMessage) As Boolean
        ...
        If DownloadResponse.Parse(oRcvMsg).Result = DownloadResult.Finished Then
            ...
            If oCurXll.ReqTeleg.IsHashValueIndicatingOkay Then
                oCurXll.ReqTeleg = CreateActiveDllContinuousReqTelegram(oCurXll.ReqTeleg, ContinueCode.Finish)
            Else
                Log.Error("The hash values differ from one another.")
                'TODO: ContinueCodeの仮想値にハッシュ値エラーを表す値を追加し、それを渡す。
        ...
        Else
            ...
            'TODO: ContinueCodeの仮想値にファイル転送エラーを表す値を追加し、それを渡す。
            oCurXll.ReqTeleg = CreateActiveDllContinuousReqTelegram(oCurXll.ReqTeleg, ContinueCode.Abort)
        End If
    ...
    Protected Overridable Function ProcOnPassiveDownloadResponseReceive(ByVal oRcvMsg As InternalMessage) As Boolean
        ...
        If DownloadResponse.Parse(oRcvMsg).Result = DownloadResult.Finished Then
            ...
            If oCurXll.ReqTeleg.IsHashValueIndicatingOkay Then
                oCurXll.ReqTeleg = CreatePassiveDllContinuousReqTelegram(oCurXll.ReqTeleg, ContinueCode.Finish)
            Else
                Log.Error("The hash values differ from one another.")
                'TODO: ContinueCodeの仮想値にハッシュ値エラーを表す値を追加し、それを渡す。
    ...
    Protected Overridable Function ProcOnActiveXllAckTelegramReceive(ByVal oRcvTeleg As ITelegram) As Boolean
        ...
        Select Case continueCode
            ...
            Case ContinueCode.Finish
                ...
                If oCurXll.Direction = XllDirection.Dll Then
                    ProcOnActiveDllComplete(oCurXll.ReqTeleg, oAckTeleg)
                Else
            ...
            Case ContinueCode.FinishWithoutStoring
                ...
                If oCurXll.Direction = XllDirection.Dll Then
                    ProcOnActiveDllComplete(oCurXll.ReqTeleg, oAckTeleg)
                Else
                    'NOTE: CreateActiveUllContinuousReqTelegramで強引なContinueCodeの電文を
                    '作成した場合にのみ、通り得る。
                    ProcOnActiveUllComplete(oCurXll.ReqTeleg, oAckTeleg)
                End If
            ...
            Case ContinueCode.Abort
                ...
                If oCurXll.Direction = XllDirection.Dll Then
                    'TODO: ハッシュ値の不一致によりここにきた場合は、
                    'ProcOnActiveDllHashValueErrorを呼ぶようにする。
                    ProcOnActiveDllTransferError(oCurXll.ReqTeleg, oAckTeleg)
                Else
                    ProcOnActiveUllTransferError(oCurXll.ReqTeleg, oAckTeleg)
                End If
            ...
            Case Else
                'NOTE: CreateActiveDllContinuousReqTelegramやCreateActiveUllContinuousReqTelegramで
                '強引なContinueCodeの電文を作成した場合にのみ、通り得る。
                Log.Info(sSeqName & "Unknown ACK received.")
    ...
    Protected Overridable Function ProcOnPassiveXllAckTelegramReceive(ByVal oRcvTeleg As ITelegram) As Boolean
        ...
        Select Case continueCode
            Case ContinueCode.Finish
                ...
                If oCurXll.Direction = XllDirection.Dll Then
                    ProcOnPassiveDllComplete(oCurXll.ReqTeleg, oAckTeleg)
                Else
            ...
            Case ContinueCode.FinishWithoutStoring
                ...
                If oCurXll.Direction = XllDirection.Dll Then
                    ProcOnPassiveDllComplete(oCurXll.ReqTeleg, oAckTeleg)
                Else
                    'NOTE: CreatePassiveUllContinuousReqTelegramで強引なContinueCodeの電文を
                    '作成した場合にのみ、通り得る。
                    ProcOnPassiveUllComplete(oCurXll.ReqTeleg, oAckTeleg)
                End If
            ...
            Case ContinueCode.Abort
                ...
                If oCurXll.Direction = XllDirection.Dll Then
                    'TODO: ハッシュ値の不一致によりここにきた場合は、
                    'ProcOnPassiveDllHashValueErrorを呼ぶようにする。
                    ProcOnPassiveDllTransferError(oCurXll.ReqTeleg, oAckTeleg)
                Else
                    ProcOnPassiveUllTransferError(oCurXll.ReqTeleg, oAckTeleg)
                End If
            ...
            Case Else
                'NOTE: CreatePassiveDllContinuousReqTelegramやCreatePassiveUllContinuousReqTelegramで
                '強引なContinueCodeの電文を作成した場合にのみ、通り得る。
                Log.Info(sSeqName & "Unknown ACK received.")
        ...
        Select Case oCurXll.ReqTeleg.ContinueCode
            ...
            Case ContinueCode.Finish
                ...
                If oCurXll.Direction = XllDirection.Dll Then
                    ...
                    End If
                    ProcOnActiveDllFinalizeError(oCurXll.ReqTeleg, oNakTeleg)
                Else
            ...
            Case Else
                'NOTE: CreateActiveDllContinuousReqTelegramやCreateActiveUllContinuousReqTelegramで
                '強引なContinueCodeの電文を作成した場合にのみ、通り得る。
                Log.Error(sSeqName & "Unknown NAK (" & nakCause.ToString() & ") received.")

                If oCurXll.Direction = XllDirection.Dll Then
                    If nakCause <> NakCauseCode.TelegramError Then
                        Log.Warn("This NAK cause is invalid for " & sSeqName & "Unknown.")
                    End If
                    ProcOnActiveDllFinalizeError(oCurXll.ReqTeleg, oNakTeleg)
                Else
                    ProcOnActiveUllFinalizeError(oCurXll.ReqTeleg, oNakTeleg)
            ...
            Case ContinueCode.Finish
                ...
                If oCurXll.Direction = XllDirection.Dll Then
                    If nakCause <> NakCauseCode.TelegramError Then
                        Log.Warn("This NAK cause is invalid for " & sSeqName & "Finish.")
                    End If
                    ProcOnPassiveDllFinalizeError(oCurXll.ReqTeleg, oNakTeleg)
                Else
...
#Region "イベント処理実装用メソッド"
    ...
    Protected Overridable Function SendReplyTelegram(ByVal oReplyTeleg As ITelegram, ByVal oSourceTeleg As ITelegram) As Boolean
        ...
        Else
            'NOTE: 不正なコマンド種別の電文を作成してoReplyTelegに渡した場合にのみあり得る。
            Return ProcOnReqTelegramReceiveCompleteBySendAck(oSourceTeleg, oReplyTeleg)
        End If
    End Function
...
#Region "表層機能カスタマイズ用仮想メソッド"
    ...
    'NOTE: 受信したファイルをどうするかは、このタイミングで確定させる（シーケンスが正常終了しない場合でも、覆してはならない）。
    ...
    '能動的DLLが成功した（ContinueCode.FinishまたはFinishWithoutStoringの転送終了REQ電文に対してACK電文を受信した）場合
    'NOTE: oXllReqTelegは、CreateActiveDllContinuousReqTelegramで生成したものである。
    'NOTE: oAckTelegは、oXllReqTeleg.ParseAsAckが生成したものである。
    Protected Overridable Sub ProcOnActiveDllComplete(ByVal oXllReqTeleg As IXllReqTelegram, ByVal oAckTeleg As IXllTelegram)
    End Sub

    '能動的DLLにて転送化けを検出（受信済みのハッシュ値と受信完了したファイルの内容に不整合を検出）して、
    'ContinueCode.Abortの転送終了REQ電文を送信した後、それに対するACK電文を受信した場合
    'NOTE: oXllReqTelegは、CreateActiveDllContinuousReqTelegramで生成したものである。
    Protected Overridable Sub ProcOnActiveDllHashValueError(ByVal oXllReqTeleg As IXllReqTelegram, ByVal oAckTeleg As IXllTelegram)
    End Sub

    '能動的DLLにて転送が失敗して、ContinueCode.Abortの転送終了REQ電文を送信した後、それに対するACK電文を受信した場合
    'NOTE: oXllReqTelegは、CreateActiveDllContinuousReqTelegramで生成したものである。
    Protected Overridable Sub ProcOnActiveDllTransferError(ByVal oXllReqTeleg As IXllReqTelegram, ByVal oAckTeleg As IXllTelegram)
    End Sub

    '能動的DLLにて転送終了REQ電文に対してNAK電文を受信した場合
    'NOTE: oXllReqTelegは、CreateActiveDllContinuousReqTelegramで生成したものである。
    Protected Overridable Sub ProcOnActiveDllFinalizeError(ByVal oXllReqTeleg As IXllReqTelegram, ByVal oNakTeleg As INakTelegram)
    End Sub
    ...
    '能動的ULLが成功した（ContinueCode.Finishの転送終了REQ電文に対してACK電文を受信した）場合
    ...
    '能動的ULLにて転送が失敗して、ContinueCode.Abortの転送終了REQ電文を送信した後、それに対するACK電文を受信した場合
    'NOTE: oXllReqTelegは、CreateActiveUllContinuousReqTelegramで生成したものである。
    Protected Overridable Sub ProcOnActiveUllTransferError(ByVal oXllReqTeleg As IXllReqTelegram, ByVal oAckTeleg As IXllTelegram)
    End Sub
    ...
    '受動的DLLが成功した（ContinueCode.FinishまたはFinishWithoutStoringの転送終了REQ電文に対してACK電文を受信した）場合
    'NOTE: oXllReqTelegは、CreatePassiveDllContinuousReqTelegramで生成したものである。
    'NOTE: oAckTelegは、oXllReqTeleg.ParseAsAckが生成したものである。
    Protected Overridable Sub ProcOnPassiveDllComplete(ByVal oXllReqTeleg As IXllReqTelegram, ByVal oAckTeleg As IXllTelegram)
    End Sub

    '受動的DLLにて転送化けを検出（受信済みのハッシュ値と受信完了したファイルの内容に不整合を検出）して、
    'ContinueCode.Abortの転送終了REQ電文を送信した後、それに対するACK電文を受信した場合
    'NOTE: oXllReqTelegは、CreatePassiveDllContinuousReqTelegramで生成したものである。
    Protected Overridable Sub ProcOnPassiveDllHashValueError(ByVal oXllReqTeleg As IXllReqTelegram, ByVal oAckTeleg As IXllTelegram)
    End Sub

    '受動的DLLにて転送が失敗して、ContinueCode.Abortの転送終了REQ電文を送信した後、それに対するACK電文を受信した場合
    'NOTE: oXllReqTelegは、CreatePassiveDllContinuousReqTelegramで生成したものである。
    Protected Overridable Sub ProcOnPassiveDllTransferError(ByVal oXllReqTeleg As IXllReqTelegram, ByVal oAckTeleg As IXllTelegram)
    End Sub

    '受動的DLLにて転送終了REQ電文に対してNAK電文を受信した場合
    'NOTE: oXllReqTelegは、CreatePassiveDllContinuousReqTelegramで生成したものである。
    Protected Overridable Sub ProcOnPassiveDllFinalizeError(ByVal oXllReqTeleg As IXllReqTelegram, ByVal oNakTeleg As INakTelegram)
    End Sub
    ...
    'NOTE: oXllReqTelegは、CreatePassiveUllContinuousReqTelegramで生成したものである。
    'NOTE: oAckTelegは、oXllReqTeleg.ParseAsAckが生成したものである。
    Protected Overridable Sub ProcOnPassiveUllComplete(ByVal oXllReqTeleg As IXllReqTelegram, ByVal oAckTeleg As IXllTelegram)
    End Sub

    '受動的ULLにて転送が失敗して、ContinueCode.Abortの転送終了REQ電文を送信した後、それに対するACK電文を受信した場合
    'NOTE: oXllReqTelegは、CreatePassiveUllContinuousReqTelegramで生成したものである。
    Protected Overridable Sub ProcOnPassiveUllTransferError(ByVal oXllReqTeleg As IXllReqTelegram, ByVal oAckTeleg As IXllTelegram)
    End Sub
