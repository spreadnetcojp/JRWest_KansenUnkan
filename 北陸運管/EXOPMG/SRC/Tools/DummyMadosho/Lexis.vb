...
Public Class Lexis
    ...
    'ウィンドウタイトル
    Public Shared FormTitle As New Sentence("多重窓口処理機向け 保守データサーバ")
    Public Shared FaultDataFormTitle As New Sentence("{1} {2} {0} {3:D}号機 異常データ編集")
    Public Shared KadoDataFormTitle As New Sentence("{1} {2} {0} {3:D}号機 稼動データ編集")
    ...
    Public Shared KadoDataManagementFileIsBroken As New Sentence("機器の稼動データのファイルサイズが異常です。\n代替措置として、機器IDや現在日時をもとにした初期値を表示します。", SentenceAttr.Warning)
    Public Shared KadoDataManagementFileReadError As New Sentence("機器の稼動データのファイル読み込みで異常が発生しました。\n代替措置として、機器IDや現在日時をもとにした初期値を表示します。", SentenceAttr.Warning)
    Public Shared KadoDataManagementFileIsLocked As New Sentence("機器の稼動データのファイルが他のプロセスにより使用中です。\n読み込みを再試行しますか？", SentenceAttr.Question)
