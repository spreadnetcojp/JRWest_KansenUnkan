...
Public Class FaultDataForm
    ...
    Public Sub New(ByVal sMonitorMachineId As String, ByVal sSourceMachineId As String, ByVal oManagerForm As MainForm)
        ...
        Dim oProf As Object() = FindProfile()
    ...
    Private Function FindProfile() As Object()
        Dim oProf As Object() _
         = If(SourceMachineId = MonitorMachineId,
              ManagerForm.UiState.Machines(MonitorMachineId).Profile,
              ManagerForm.UiState.Machines(MonitorMachineId).TermMachines(SourceMachineId).Profile)
        Return oProf
    End Function
    ...
    Private Function CreateInitialValue(ByVal oField As XlsField) As String
        Select Case oField.MetaName
            ...
            Case "基本ヘッダー コーナー"
                Dim oProf As Object() = FindProfile()
                Return CStr(oProf(Config.MachineProfileFieldNamesIndices("CORNER_CODE")))
            Case "基本ヘッダー 号機"
                If SourceMachineId = MonitorMachineId Then
                    Dim oProf As Object() = FindProfile()
                    Return CStr(oProf(Config.MachineProfileFieldNamesIndices("UNIT_NO")))
                Else
                    Return "0"
                End If
            Case "基本ヘッダー シーケンスNo"
            ...
            Case "号機番号"
                ...
                Else
                    Dim oProf As Object() = FindProfile()
                    Return CInt(oProf(Config.MachineProfileFieldNamesIndices("UNIT_NO"))).ToString("D2")
                End If
            Case "通路方向"
                If SourceMachineId = MonitorMachineId Then
                    Return FaultDataUtil.CreatePassDirectionValue(ManagerForm.UiState.Machines(MonitorMachineId).LatchConf)
                Else
        ...
        For i As Integer = 0 To FaultDataUtil.Fields.Length - 1
            ...
            Select Case FaultDataUtil.Fields(i).MetaType
                ...
                Case "FaultDataErrorCode"
                    oCombo.DataSource = If(SourceMachineId = MonitorMachineId, Config.KsbFaultDataErrorCodeItems, Config.FaultDataErrorCodeItems)
                    oCell.ReadOnly = False
                Case Else
            ...
            Using oOutputStream As New FileStream(FaultDataRewriteFileDialog.FileName, mode, FileAccess.Write, FileShare.Read)
                If mode = FileMode.OpenOrCreate Then
                    ...
                    If fileLen < RecordLength Then
                        ExUpboundFileHeader.WriteToStream(&HB6, 1, RecordLength, now, oOutputStream)
                    Else
                        oOutputStream.Seek(0, SeekOrigin.Begin)  'OPT: 不要かもしれない。
                        Dim recCount As Integer = CInt((fileLen \ RecordLength) - 1)
                        ExUpboundFileHeader.WriteToStream(&HB6, recCount, RecordLength, now, oOutputStream)
            ...
            Using oOutputStream As New FileStream(FaultDataAppendFileDialog.FileName, mode, FileAccess.Write, FileShare.Read)
                ...
                If mode = FileMode.OpenOrCreate Then
                    ...
                    Else
                        ...
                        ExUpboundFileHeader.WriteToStream(&HB6, totalRecCount, RecordLength, now, oOutputStream)
    ...
    Private Sub BaseHeaderSetButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BaseHeaderSetButton.Click
        ...
        SetValue("基本ヘッダー 号機", If(SourceMachineId = MonitorMachineId, CStr(oProf(Config.MachineProfileFieldNamesIndices("UNIT_NO"))), "0"))
    ...
    Private Sub AllHeadersSetButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles AllHeadersSetButton.Click
        ...
        SetValue("号機番号", If(SourceMachineId = MonitorMachineId, "00", CInt(oProf(Config.MachineProfileFieldNamesIndices("UNIT_NO"))).ToString("D2")))
        SetValue("通路方向", FaultDataUtil.CreatePassDirectionValue(If(SourceMachineId = MonitorMachineId, ManagerForm.UiState.Machines(MonitorMachineId).LatchConf, ManagerForm.UiState.Machines(MonitorMachineId).TermMachines(SourceMachineId).LatchConf)))
    End Sub
    ...
    Private Sub ErrorTextsSetButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ErrorTextsSetButton.Click
        ...
        If SourceMachineId = MonitorMachineId Then
            'Dim oRows As DataRow() = Config.KsbFaultDataErrorCodeItems.Select("Key = '" & GetValue("エラーコード") & "'")
            ...
            Try
                ...
                If Config.KsbFaultDataErrorOutlines.TryGetValue(sCode, sText) = False Then
                    sText = "\0"
                End If
                SetValue("異常項目 表示データ", sText)
                SetByteCountValue("異常項目")

                If Config.KsbFaultDataErrorLabels.TryGetValue(sCode, sText) = False Then
                    sText = "\0"
                End If
                SetValue("４文字表示 表示データ", sText)
                SetByteCountValue("４文字表示")

                If Config.KsbFaultDataErrorDetails.TryGetValue(sCode, sText) = False Then
                    sText = "\0"
                End If
                SetValue("可変表示部 表示データ", sText)
                SetByteCountValue("可変表示部")

                If Config.KsbFaultDataErrorGuidances.TryGetValue(sCode, sText) = False Then
                    sText = "\0"
                End If
