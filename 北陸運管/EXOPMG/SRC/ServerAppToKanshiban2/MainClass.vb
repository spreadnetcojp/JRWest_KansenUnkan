...
#Region "メソッド"
    ''' <summary>
    ''' 対監視盤利用データ通信プロセスのメイン処理。
    ''' </summary>
    ''' <remarks>
    ''' 対監視盤利用データ通信プロセスのエントリポイントである。
    ''' </remarks>
    <STAThread()> _
    Public Shared Sub Main()
        Dim m As New Mutex(False, "ExOpmgServerAppToKanshiban2")
        If m.WaitOne(0, False) Then
            Try
                ...
                Log.Init(sLogBasePath, "ToKanshiban2")
                Log.Info("プロセス開始")
