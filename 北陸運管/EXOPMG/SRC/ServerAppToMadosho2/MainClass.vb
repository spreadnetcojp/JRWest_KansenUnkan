...
#Region "メソッド"
    ''' <summary>
    ''' 対窓処利用データ通信プロセスのメイン処理。
    ''' </summary>
    ''' <remarks>
    ''' 対窓処利用データ通信プロセスのエントリポイントである。
    ''' </remarks>
    <STAThread()> _
    Public Shared Sub Main()
        Dim m As New Mutex(False, "ExOpmgServerAppToMadosho2")
        If m.WaitOne(0, False) Then
            Try
                ...
                Log.Init(sLogBasePath, "ToMadosho2")
                Log.Info("プロセス開始")
