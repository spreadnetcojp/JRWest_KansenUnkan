...
''' 外部から電文を取り込むクラス。
''' </summary>
Public Class NkTelegramImporter
    Implements ITelegramImporter

#Region "定数"
    'NOTE: 下記の条件を満たす値を設定すること。
    'MaxReceiveSize > MinAllocSize >= NkTelegram.ObjPos
    Private Const MinAllocSize As Integer = 256
    Private Const MaxReceiveSize As Integer = 1024 * 1024 * 1024
#End Region
...
#Region "メソッド"
    ...
    Public Function GetTelegramFromBytes( _
       ByVal aBytes As Byte()) As NkDodgyTelegram

        Dim minReceiveSize As Integer = NkTelegram.ObjPos
        If aBytes.Length < minReceiveSize Then
            Log.Error("The bytes is too short as NkTelegram.")
            Return Nothing
        End If

        Dim objSize As UInteger = Utility.GetUInt32FromLeBytes4(aBytes, NkTelegram.ObjSizePos)
        If objSize > Integer.MaxValue - NkTelegram.ObjPos Then
            Log.Error("ObjSize of the telegram is too large.")
            Return Nothing
        End If

        Dim telegLen As Integer = NkTelegram.ObjPos + CInt(objSize)
        ...
        Return New NkDodgyTelegram(aTelegBytes)
    End Function
    ...
    Public Function GetTelegramFromStream( _
       ByVal oStream As Stream) As NkDodgyTelegram

        Dim minReceiveSize As Integer = NkTelegram.ObjPos
        Debug.Assert(MinAllocSize >= minReceiveSize)
        Debug.Assert(MaxReceiveSize > MinAllocSize)

        Dim telegLen As Integer = minReceiveSize
        Dim aBytes As Byte() = New Byte(MinAllocSize - 1) {}
        ...
        Do
            ...
            If offset = telegLen Then
                ...
                telegLen = NkTelegram.ObjPos + CInt(objSize)
                ...
                If telegLen > MinAllocSize Then
                    If telegLen > MaxReceiveSize Then
        ...
        Return New NkDodgyTelegram(aBytes)
    End Function
    ...
    Public Function GetTelegramFromSocket( _
       ...
       Optional ByVal telegLoggingMaxLength As Integer = 0) As NkDodgyTelegram
