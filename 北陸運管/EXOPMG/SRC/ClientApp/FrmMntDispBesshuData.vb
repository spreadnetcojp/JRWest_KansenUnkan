...
''' 【別集札データ確認　画面クラス】
''' </summary>
Public Class FrmMntDispBesshuData
    Inherits FrmBase
...
#Region " Windows フォーム デザイナで生成されたコード "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(FrmMntDispBesshuData))
        ...
        Me.lblToday.Text = "2013/08/21(水)  12:49"
        '
        'ImageList1
        '
        ...
        Me.shtMain.Size = New System.Drawing.Size(970, 463)
        ...
        '
        'FrmMntDispBesshuData
        '
        Me.AutoScaleBaseSize = New System.Drawing.Size(5, 12)
        Me.BackColor = System.Drawing.SystemColors.ControlLight
        Me.ClientSize = New System.Drawing.Size(1014, 732)
        Me.Name = "FrmMntDispBesshuData"
        Me.Text = "運用端末 V1.00"
...
#Region "宣言領域（Private）"
    ...
    ''' 出力用テンプレートファイル名
    ''' </summary>
    Private ReadOnly LcstXlsTemplateName As String = "別集札データ.xls"

    ''' <summary>
    ''' 出力時用テンプレートシート名
    ''' </summary>
    Private ReadOnly LcstXlsSheetName As String = "別集札データ"

    ''' <summary>
    ''' 画面名
    ''' </summary>
    Private ReadOnly LcstFormTitle As String = "別集札データ確認"
    ...
    ''' </summary>
    Private ReadOnly LcstSortCol() As Integer = {-1, 0, -1, 3, 4, -1, -1, -1, -1, -1, -1, -1}

    ''' <summary>
    ''' 帳票出力対象列の割り当て
    ''' （検索した別集札データに対し帳票出力列を定義）
    ''' </summary>
    Private ReadOnly LcstPrntCol() As Integer = {1, 2, 3, 4, 6, 7, 8, 9, 10, 11}
...
#Region "イベント"
    ...
    ''' フォームロード
    ''' </summary>
    Private Sub FrmMntDispBesshuData_Load(ByVal sender As Object, ByVal e As System.EventArgs) _
    Handles MyBase.Load
        LfWaitCursor()
        Try
    ...
    Handles btnKensaku.Click
        ...
        Try
            ...
            nRtn = BaseSqlDataTableFill(sSql, dtData)
            Select Case nRtn
                Case -9             'ＤＢオープンエラー
                ...
                Case Else
                    '上限チェック
                    If Convert.ToInt64(dtData.Rows(0)(0)) > nMaxCount Then
                        AlertBox.Show(Lexis.HugeRecordsFound, nMaxCount.ToString())
                        cmbEki.Select()
                        Exit Sub
                    ElseIf Convert.ToInt64(dtData.Rows(0)(0)) = 0 Then
...
#Region "メソッド（Private）"
    ...
    Private Sub LfSearchTrue()
        ...
        End If
        If bEnabled Then
            If ((cmbEki.SelectedIndex < 0) Or _
                (cmbMado.SelectedIndex < 0) Or _
    ...
    ''' [検索用SELECT文字列取得]
    ''' </summary>
    ''' <param name="slcSQLType">SQL取得区分 1:件数取得用 2:検索用</param>
        ...
        Try
            ...
            Select Case slcSQLType
                Case slcSQLType.SlcCount
                    '件数取得--------------------------
                    sBuilder.AppendLine(" SELECT COUNT(1) FROM V_BESSHU_DATA ")
                Case slcSQLType.SlcDetail
                    '取得項目--------------------------
                    sBuilder.AppendLine(" SELECT STATION_CODE ")
                    sBuilder.AppendLine("      , STATION_NAME ")
                    sBuilder.AppendLine("      , CORNER_NAME ")
                    sBuilder.AppendLine("      , SUBSTRING(PROCESSING_TIME,1,4)+'/'+SUBSTRING(PROCESSING_TIME,5,2)+'/'+SUBSTRING(PROCESSING_TIME,7,2)+' '+ ")
                    sBuilder.AppendLine("        SUBSTRING(PROCESSING_TIME,9,2)+':'+SUBSTRING(PROCESSING_TIME,11,2)+':'+SUBSTRING(PROCESSING_TIME,13,2) ")
                    sBuilder.AppendLine("       AS YMDHMS")
                    sBuilder.AppendLine("      , TICKET_NAME ")
                    sBuilder.AppendLine("      , SECTION ")
                    sBuilder.AppendLine("      , DEPARTURE_STATION ")
                    sBuilder.AppendLine("      , ARRIVAL_STATION ")
                    sBuilder.AppendLine("      , UNIT_NO ")
                    sBuilder.AppendLine("      , PASSAGE_NAME ")
                    sBuilder.AppendLine("      , CAUSE_NAME ")
                    sBuilder.AppendLine("      , RETURN_NAME ")
                    sBuilder.AppendLine("      , TICKET_NO ")
                    sBuilder.AppendLine("  FROM V_BESSHU_DATA ")
            End Select
