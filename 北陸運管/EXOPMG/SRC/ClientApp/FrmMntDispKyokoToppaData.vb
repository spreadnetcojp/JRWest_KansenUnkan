...
'   0.1      2015/02/23  (NES)金沢  帳票出力不具合対応（日付形式修正）
' **********************************************************************
Option Strict On
Option Explicit On
...
''' 【強行突破検出データ確認　画面クラス】
''' </summary>
Public Class FrmMntDispKyokoToppaData
    Inherits FrmBase
...
#Region " Windows フォーム デザイナで生成されたコード "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(FrmMntDispKyokoToppaData))
        ...
        Me.lblToday.Text = "2013/08/21(水)  12:53"
        '
        'ImageList1
        '
        ...
        Me.shtMain.Size = New System.Drawing.Size(970, 464)
        ...
        '
        'FrmMntDispKyokoToppaData
        '
        Me.AutoScaleBaseSize = New System.Drawing.Size(5, 12)
        Me.BackColor = System.Drawing.SystemColors.ControlLight
        Me.ClientSize = New System.Drawing.Size(1014, 732)
        Me.Name = "FrmMntDispKyokoToppaData"
        Me.Text = "運用端末 V1.00"
...
#Region "宣言領域（Private）"
    ...
    ''' 出力用テンプレートファイル名
    ''' </summary>
    Private ReadOnly LcstXlsTemplateName As String = "強行突破検出データ.xls"

    ''' <summary>
    ''' 出力時用テンプレートシート名
    ''' </summary>
    Private ReadOnly LcstXlsSheetName As String = "強行突破検出データ"

    ''' <summary>
    ''' 画面名
    ''' </summary>
    Private ReadOnly FormTitle As String = "強行突破検出データ確認"
    ...
    ''' </summary>
    Private ReadOnly LcstSortCol() As Integer = {6, -1, 2, -1, -1, -1}
    ''' <summary>
    ''' 帳票出力対象列の割り当て
...
#Region "イベント"
    ...
    ''' フォームロード
    ''' </summary>
    Private Sub FrmMntDispKyokoToppaData_Load(ByVal sender As Object, ByVal e As System.EventArgs) _
    Handles MyBase.Load
        LfWaitCursor()
        Try
    ...
    ''' ソート
    ''' </summary>
    Private Sub SheetSort(ByRef sheetTarget As GrapeCity.Win.ElTabelleSheet.Sheet, ByVal intKeyColumn As Integer, ByVal sortOrder _
                          As GrapeCity.Win.ElTabelleSheet.SortOrder)
...
#Region "メソッド（Private）"
    ...
    Private Function LfGetSelectString(ByVal slcSQLType As SlcSQLType) As String
        ...
        Try
            ...
            Dim sFrom As StringBuilder = New StringBuilder()
            Dim sTo As StringBuilder = New StringBuilder()
            Dim sEki As String
            Select Case slcSQLType
                Case slcSQLType.SlcCount
                    '件数取得項目--------------------------
                    sSql.AppendLine("SELECT COUNT(1) FROM V_KYOKO_TOPPA_DATA")
                Case slcSQLType.SlcDetail
                    ...
                    sSql.AppendLine(" AS YMDHMS, UNIT_NO, PASSAGE_NAME,")
                    sSql.AppendLine("  ERROR_NAME,STATION_CODE FROM V_KYOKO_TOPPA_DATA")
            End Select
            'Where句生成--------------------------
    ...
    Private Sub LfXlsStart(ByVal sPath As String)
        ...
        Try
            With XlsReport1
                ...
                '----Ver0.1　日付出力形式対応　MOD　START------------------------------
                .Cell("C4").Value = Lexis.TimeSpan.Gen( _
                                                  Replace(Replace(Replace(dtpYmdFrom.Text, "年", "/"), "月", "/"), "日", ""), _
                                                  dtpHmFrom.Text, Replace(Replace(Replace(dtpYmdTo.Text, "年", "/"), "月", "/"), "日", ""), dtpHmTo.Text)
                '----Ver0.1　日付出力形式対応　MOD　END------------------------------
                ' 配信対象のデータ数を取得します
