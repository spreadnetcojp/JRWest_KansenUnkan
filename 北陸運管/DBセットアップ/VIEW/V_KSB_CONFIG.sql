...
/****** Object:  View [dbo].[V_KSB_CONFIG]    Script Date: 09/04/2014 10:31:10 ******/
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[V_KSB_CONFIG]'))
DROP VIEW [dbo].[V_KSB_CONFIG]
GO

USE [EXOPMG]
GO

/****** Object:  View [dbo].[V_KSB_CONFIG]    Script Date: 09/04/2014 10:31:07 ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[V_KSB_CONFIG]
AS
SELECT                  TOP (100) PERCENT T.RAIL_SECTION_CODE + T.STATION_ORDER_CODE AS STATION_CODE, V1.STATION_NAME, T.CORNER_CODE, 
                                  ...
                                  AS NYUJYOTYUSI, (CASE T .SYSTEMSYOGAI WHEN '00' THEN '切' WHEN '01' THEN '入' END) AS SYSTEMSYOGAI, 
...
FROM                     dbo.D_KSB_CONFIG AS T INNER JOIN
                                  dbo.v_station_mast AS V1 ON T.RAIL_SECTION_CODE + T.STATION_ORDER_CODE = V1.STATION_CODE INNER JOIN
...
Begin DesignProperties = 
   ...
   End
End
' , @level0type=N'SCHEMA',@level0name=N'dbo', @level1type=N'VIEW',@level1name=N'V_KSB_CONFIG'
GO

EXEC sys.sp_addextendedproperty @name=N'MS_DiagramPaneCount', @value=1 , @level0type=N'SCHEMA',@level0name=N'dbo', @level1type=N'VIEW',@level1name=N'V_KSB_CONFIG'
GO
